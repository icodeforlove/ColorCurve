...
Filters.ApplyFilter = function(idatasource, idataout) {
	...
	document.getElementById('settings').innerHTML = JSON.stringify({
		r: Array.prototype.concat.apply([],CC1.points.map(function (point) {return [parseFloat(point.x.toFixed(4)), parseFloat(point.y.toFixed(4))]})),
		g: Array.prototype.concat.apply([],CC2.points.map(function (point) {return [parseFloat(point.x.toFixed(4)), parseFloat(point.y.toFixed(4))]})),
		b: Array.prototype.concat.apply([],CC3.points.map(function (point) {return [parseFloat(point.x.toFixed(4)), parseFloat(point.y.toFixed(4))]}))
	}).replace(/"/g, '');
